...
class Main {
  ...
  async init(context) {
    ...
    const host = await geAjax(`${window.API_PREFIX}/api/console/host/imageHost`, 'GET')
    this.fileSystem = host.imageHost
    this.applyInfo = await geAjax(`${window.API_PREFIX}/api/iot-cds/cds/configurationDesignStudioForPreview/${id}`, 'GET')
    if (!this.applyInfo) {
      return
    }
