...
async function getLastData(deviceParams, fileSystem,mainProcess) {
  ...
  const res = await geAjax(`${window.API_PREFIX}/api/v2/persist/tsdb/point/last`, 'POST', JSON.stringify(paramIds))
...
async function getSubscribeInfos(deviceParams) {
  ...
  let data = await geAjax(`${window.API_PREFIX}/api/pubsub/subscribe`, 'POST', JSON.stringify(params))
